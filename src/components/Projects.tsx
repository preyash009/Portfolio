...
const projects = [
  {
    title: 'Playbook365',
    description: 'A comprehensive e-commerce solution built with Laravel backend and Vue.js frontend. Features include payment integration, inventory management, and admin dashboard with real-time analytics.',
...
const ProjectCard = ({ project, index }: { project: typeof projects[0]; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className={`glass-morphism rounded-2xl overflow-hidden group hover:scale-105 transition-all duration-300 ${
        project.featured ? 'ring-2 ring-purple-500/50' : ''
      }`}
    >
      {project.featured && (
        <div className="bg-gradient-to-r from-purple-500 to-blue-500 text-white text-xs font-bold px-3 py-1 absolute top-4 right-4 rounded-full z-10">
          Featured
        </div>
      )}
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tech.map((tech) => (
            <span 
              key={tech} 
              className="bg-gray-800/50 px-3 py-1 rounded-full text-xs text-gray-300 border border-gray-700 hover:border-purple-500 transition-colors"
            >
              {tech}
            </span>
          ))}
  ...
  return (
    <section id="projects" className="py-20 pt-20 relative bg-black/30" ref={ref}>
      <div className="container mx-auto px-6">
        ...
        >
          <a
            href="https://github.com/preyash009"
            className="inline-flex items-center gap-3 glass-morphism px-8 py-4 rounded-full hover:scale-105 transition-all duration-300 group"
            target='_blank'
          >
